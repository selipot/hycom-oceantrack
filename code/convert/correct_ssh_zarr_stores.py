...
for i in range(1,13):
    ...
    zarr.consolidate_metadata(datadir + 'hycom12-ssh-' + str(i) + '-rechunked.zarr')

    dsr = xr.open_zarr(datadir + 'hycom12-ssh-'+str(i)+'-rechunked.zarr',consolidated=True)
    ...
    # mask the values based on min max of ssh? should work
    dsr_masked = dsr_masked.where(np.logical_and(dsr_masked.ssh>=dsr.ssh.valid_range[0],dsr_masked.ssh<=dsr.ssh.valid_range[1]))
    ...
    dsr_masked.to_zarr(outdir + 'hycom12-ssh-' + str(i) + '-rechunked-corr.zarr',mode="w")

    print('Done correcting ssh zarr' + str(i))
